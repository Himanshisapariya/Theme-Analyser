...
function initPromoDrawerSwatches() {
  ...
  function scheduleHideActivatedPromoTags(cartData) {
    ...
    _hidePromoTimer = setTimeout(() => hideActivatedPromoTags(cartData), 50);
  }

  // Replace broad MutationObserver (fired 90+ times on Rebuy init) with
  // targeted cart/Rebuy events so /cart.js is fetched only on real cart changes.
  document.addEventListener('rebuy:cart.updated',   () => { initRebuyPromo(); scheduleHideActivatedPromoTags(); });
