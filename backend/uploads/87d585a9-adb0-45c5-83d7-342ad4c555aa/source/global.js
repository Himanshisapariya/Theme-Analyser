...
class MenuDrawer extends HTMLElement {
	...
	bindEvents() {
		...
		this.querySelectorAll(".menu-drawer__submenu-toggle").forEach((button) =>
			button.addEventListener("click", this.onSubmenuToggleClick.bind(this))
		);
		this.querySelectorAll("button").forEach((button) => {
			...
			if (this.querySelector(".header__localization-lang-button") === button)
				return;
			if (button.classList.contains("menu-drawer__submenu-toggle")) return;
	...
	onKeyUp(event) {
		...
		const openDetailsElement = event.target.closest("details[open]");
		const openSubmenuWrapper = event.target.closest(".menu-item-has-submenu.menu-opening");
		if (!openDetailsElement && !openSubmenuWrapper) return;

		if (openDetailsElement === this.mainDetailsToggle) {
			this.closeMenuDrawer(this.mainDetailsToggle.querySelector("summary"));
		} else if (openSubmenuWrapper) {
			this.closeSubmenu(openSubmenuWrapper);
		} else if (openDetailsElement) {
	...
	onSubmenuToggleClick(event) {
		...
		const wrapper = button.closest('.menu-item-has-submenu');
		const isOpen = wrapper.classList.contains('menu-opening');
		if (isOpen) {
			this.closeSubmenu(wrapper);
		} else {
			wrapper.classList.add('menu-opening');
			const closeButton = wrapper.querySelector('button.menu-drawer__close-button');
			if (closeButton) trapFocus(wrapper, closeButton);
		}
	}
	...
	closeMenuDrawer(event, elementToFocus = false) {
		if (event !== undefined) {
			...
			this.mainDetailsToggle.querySelectorAll(".menu-item-has-submenu").forEach((wrapper) => {
	...
	onCloseButtonClick(event) {
		const detailsElement = event.currentTarget.closest("details, .menu-item-has-submenu");
...
function initializeQuickView(container) {
	...
	function extractVariantParts(altText) {
		...
		return matchSegment.replace("variants:", "").split("*").map(v => v.trim()).filter(Boolean);
	}

	function updateModelSizeFromActiveSlide() {
		const checkedSwatch = Array.from(colorSwatches).find(input => input.checked);
		const modelSizing = checkedSwatch?.dataset?.modelSizing || "";
		if (modelSizeTextEl) modelSizeTextEl.textContent = modelSizing;
	}
