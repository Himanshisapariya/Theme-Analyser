	...
	customElements.define(
	  ...
	  class QuickAddModal extends ModalDialog {
		constructor() {
		  ...
		  // fix - #560: focus trap — bound once so add/remove works correctly
		  this._qvFocusTrap = (e) => {
			if (e.key !== 'Tab') return;
			...
			if (!this.contains(document.activeElement)) { e.preventDefault(); first.focus(); return; }
		...
		hide(preventFocus = false) {
		  ...
		  document.removeEventListener('keydown', this._qvFocusTrap);
		  super.hide();
		...
		show(opener) {
		  ...
		  fetch(opener.getAttribute("data-product-url"))
           ...
           /** Code added by starapps **/
			  ...
			  super.show(opener);
			  document.addEventListener('keydown', this._qvFocusTrap);
...
(function ($) {
	const initProductAccordion = () => {
		// Native <button> handles Enter/Space — no manual keydown handler needed
		$(".about__accordion-title").off("click").on("click", function () {
			...
			$allItems.each(function () {
				...
				$(this).find("button[aria-expanded]").attr("aria-expanded", String(isCurrent ? !isActive : false));
			});
		});
	};

    // Wait until ".about__accordion-item" button element is found
    const waitForAccordionTitle = function () {
        return new Promise(function (resolve, reject) {
            const startTime = Date.now();
            const checkElement = function () {
                if ($(".about__accordion-item").length) {
                    resolve();
