...
class CustomerAddresses {
	...
	_toggleExpanded(target, items) {
		...
		if (target.getAttribute("data-type") === "reset") {
			...
			items.forEach((el) => {
				if (
					target.getAttribute("id").slice(15) ===
					el.getAttribute("id").slice(12)
				) {
					el.setAttribute(
						attributes.expandedTarget,
				...
				if (el.getAttribute(attributes.expandedTarget) === "false") {
					setTimeout(function () {
						$("html, body").animate(
							{
								scrollTop: target.closest(".address-list__item")
									.previousElementSibling.offsetTop,
							},
							700
						);
					}, 100);
...
// Add this at the very end of customer.js
