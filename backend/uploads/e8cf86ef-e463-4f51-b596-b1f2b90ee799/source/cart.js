...
class CartItems extends HTMLElement {
	constructor() {
		...
		this.addEventListener("click", (event) => {
			// QuantityInput (global component) already increments and dispatches a change
			// event on these buttons. We skip handling clicks that were already processed
			// there (default is prevented) to avoid double-updating quantities.
		...
		// Listen for custom cart refresh events (from GWP, etc.)
...
renderCartSections(cart) {
  ...
  Promise.all(renderPromises).then(() => {
    // Trigger the cart display update from main-cart-subtotal
    this.updateSubtotalDisplay(cart);
    // Re-init GWP logic on every cart render so the inline snippet keeps working
    initGwp(cart);
    ...
    if (cartIconBubble) {
      fetch(`${routes.cart_url}?section_id=cart-icon-bubble`)
        .then(response => {
          if (!response.ok) {
            throw new Error(`Cart icon bubble fetch error! status: ${response.status}`);
          }
          return response.text();
        })
        .then(html => {
          cartIconBubble.innerHTML = this.getSectionInnerHTML(html, '.shopify-section');
        })
        .catch(error => {
...
updateLiveRegions(line, message) {
	...
	enableLoading(line) {
		...
		// Disable cart checkout buttons during loading
		const cartCtas = document.querySelectorAll(".cart__ctas");
		cartCtas.forEach((cta) => {
	...
	disableLoading(line) {
		...
		// Re-enable cart checkout buttons after loading
		const cartCtas = document.querySelectorAll(".cart__ctas");
		cartCtas.forEach((cta) => {
...
function initGwp(cartFromEvent) {
  ...
  const updateUI = (cart) => {
    ...
    if (!qualified || !hasPaidItem) {
      statusEl && (statusEl.innerHTML = '<span>Keep shopping to claim your free gift.</span>');
...
if (button) {
  ...
  if (button) {
    button.addEventListener('click', () => {
      if (!latestCart || processing) return;

      const qualified = latestCart.total_price >= threshold;
      const hasPaidItem = latestCart.items.some((item) => (item.final_line_price || item.price) > 0);
      ...
      // Check if GWP item already exists in cart using the new isGift function
      const existingGwpItem = latestCart.items.find(item => 
        isGift(item) && item.variant_id === variantId
      );
      
      if (existingGwpItem) {
        console.log('GWP item already exists in cart, skipping addition');
        processing = false;
        return;
      }
...
// Run once on load in case the page was loaded with a stale gift
